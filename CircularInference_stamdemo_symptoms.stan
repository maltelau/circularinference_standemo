...
parameters{
    ...
    real<lower=-aOthers> aOthersSymptoms;
    real<lower=0> aSelf;
    vector<lower=-aSelf>[P] aSelfP;
    real<lower=-aSelf> aSelfSymptoms;
...
transformed parameters {
    ...
    ws = inv_logit(wSelf + wSelfP[Participant] + wSelfSymptoms * Symptoms)/2+.5;
    
    # weighting for prior information
    wo = inv_logit(wOthers + wOthersP[Participant] + wOthersSymptoms * Symptoms)/2+.5;
